...
const CONFIG = {
    ...
    FULLSCREEN_FEATURE_KEY: 'fullscreen_feature_enabled',
...
function toggleFullscreen() {
    // Перевіряємо чи дозволений fullscreen
    if (!isFullscreenFeatureEnabled()) {
        alert('Функція повноекранного режиму вимкнена в налаштуваннях');
        return;
    }
...
/**
 * Перевірити чи увімкнена функція fullscreen взагалі
 */
function isFullscreenFeatureEnabled() {
    const enabled = localStorage.getItem(CONFIG.FULLSCREEN_FEATURE_KEY);
    return enabled === null || enabled === 'true'; // За замовчуванням увімкнено
}

/**
 * Перемкнути можливість використання fullscreen
 */
function toggleFullscreenFeature() {
    const currentState = isFullscreenFeatureEnabled();
    const newState = !currentState;
    
    localStorage.setItem(CONFIG.FULLSCREEN_FEATURE_KEY, newState.toString());
    
    const toggle = document.getElementById('fullscreen-feature-toggle');
    if (toggle) {
        if (newState) {
            toggle.classList.add('active');
        } else {
            toggle.classList.remove('active');
            // Якщо вимикаємо fullscreen, виходимо з повноекранного режиму
...
function initDoubleClickFullscreen() {
    document.addEventListener('click', function(e) {
        // Перевірити чи функції увімкнені
        if (!isFullscreenFeatureEnabled() || !isDblClickFullscreenEnabled()) return;
